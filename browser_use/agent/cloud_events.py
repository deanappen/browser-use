...
class UpdateAgentTaskEvent(BaseEvent):
    ...
    # UserFeedbackType enum value as string
    ...
    def from_agent(cls, agent) -> 'UpdateAgentTaskEvent':
        ...
        return cls(
            ...
            stopped=agent.state.stopped if hasattr(
                agent.state, 'stopped') else False,
            paused=agent.state.paused if hasattr(
                agent.state, 'paused') else False,
            done_output=done_output,
            finished_at=datetime.now(
                timezone.utc) if agent.state.history and agent.state.history.is_done() else None,
            agent_state=agent.state.model_dump() if hasattr(
                agent.state, 'model_dump') else {},
...
class CreateAgentOutputFileEvent(BaseEvent):
    ...
    content_type: str | None = Field(
        None, max_length=100)  # MIME type for file uploads
    ...
    def validate_file_size(cls, v: str | None) -> str | None:
        ...
        if estimated_size > MAX_FILE_CONTENT_SIZE:
            raise ValueError(
                f'File content exceeds maximum size of {MAX_FILE_CONTENT_SIZE / 1024 / 1024}MB')
        return v
...
class CreateAgentStepEvent(BaseEvent):
    ...
    screenshot_url: str | None = Field(
        None, max_length=MAX_FILE_CONTENT_SIZE)  # ~50MB for base64 images
    ...
    def validate_screenshot_size(cls, v: str | None) -> str | None:
        ...
        if ',' in v:
            ...
            if estimated_size > MAX_FILE_CONTENT_SIZE:
                raise ValueError(
                    f'Screenshot content exceeds maximum size of {MAX_FILE_CONTENT_SIZE / 1024 / 1024}MB')
        return v
    ...
    def from_agent_step(
        ...
        current_state = model_output.current_state if hasattr(
            model_output, 'current_state') else None
        ...
        for action in actions_data:
            if "click_element_by_index" in action:
                ...
                click_coordinates.append(
                    browser_state_summary.selector_map[index].coordinates)
        ...
        print(
            f'====sb1: {coordinates_list.__len__()}',
        )

        print(
            f'====sb2: {click_coordinates.__len__()}'
        )
    ...
    def from_agent(cls, agent) -> 'CreateAgentTaskEvent':
        ...
        return cls(
            ...
            started_at=datetime.fromtimestamp(
                agent._task_start_time, tz=timezone.utc),
            finished_at=None,
            user_feedback_type=None,
            user_comment=None,
            gif_url=None,
